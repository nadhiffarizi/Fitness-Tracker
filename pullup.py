...
class FullBodyPoseEmbedder(object):
    ...
    def _distance_in_y(self, arr):
        '''get the distance in y axis. arr argument is expected to be (N, 3)'''
    ...
    def _get_pose_distance_embedding(self, landmarks):
        ...
        embedding = np.apply_along_axis(self._convert_distancesxyz_magnitude,
                                        axis=1, arr=embedding)

        # add eye to wrist distance in y coordinate
        distances_y = np.array([self._get_distance_by_names(landmarks, 'left_eye', 'left_wrist'),
                                self._get_distance_by_names(landmarks, 'right_eye', 'right_wrist'),
        ...
        distances_y = self._distance_in_y(distances_y)

        embedding = np.append(embedding, distances_y, axis=0)
...
class KNNClassifier(object):
    '''This is a class for custom KNN. We implement KNN using weighted-KNN
      ...
      and top-K distances to each data points in the dataset'''
    ...
    def __call__(self):
        ...
        classify_dict = self.__threshold__(sorted_result, classify_dict)
        return classify_dict, sorted_result
    ...
    def __threshold__(self, sorted_result, classify_dict):
        ...
        for distance, class_code in sorted_result:
            if class_code == 1 and distance > 85:
                # this condition we don't confident with the 'up' classification
                classify_dict["up"] -= 1
            elif class_code == 0 and distance > 60:
    ...
    def __weight_features__(self, distances):
        ...
        '''array of features = ['center','lshoulder_lelbow', 
                                  'rshoulder_relbow','lelbow_lwrist', 
                                  'relbow_rwrist','lhip_lknee', 
                                  'rhip_rknee','lknee_lankle', 
                                  'rknee_rankle','lshoulder_lwrist', 
                                  'rshoulder_rwrist','lhip_lwrist', 
                                  'rhip_rwrist','lshoulder_lankle',
                                  'rshoulder_rankle',
                                  'lelbow_relbow','lknee_rknee',
                                  'lwrist_rwrist','lankle_rankle',
                                  'leye_lwrist_y', 'reye_rwrist_y',
                                  'leye_lshoulder_y', 'reye_rshoulder_y',
                                  'agl_lshoulder_lelbow_lwrist', 'agl_rshoulder_relbow_rwrist', 
                                  'agl_lshoulder_lhip_lknee', 'agl_rshoulder_rhip_rknee',
                                  'agl_lhip_lknee_lankle', 'agl_rhip_rknee_rankle', 'agl_lelbow_lshoulder_lhip',
                                  'agl_relbow_rshoulder_rhip'] '''
